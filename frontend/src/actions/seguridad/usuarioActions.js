...
export const listaComboPerfil = () => async (dispatch, getState) => {
  try {
    dispatch({
...
export const listaComboSupervisor = () => async (dispatch, getState) => {
  try {
    dispatch({
      type:SUPERVISOR_LIST_REQUEST,
    });
    const {
      userLogin: { userInfo },
    } = getState();
    ...
    dispatch({
      type:SUPERVISOR_LIST_SUCCESS,
      payload: data,
    });
  } catch (error) {
    dispatch({
      type:SUPERVISOR_LIST_FAIL,
      payload:
        error.response && error.response.data.message
          ? error.response.data.message
          : error.message,
...
export const updateCooperativa =
  ...
  (cooperativa) => async (dispatch, getState) => {
    try {
      ...
      const { data } = await axios.put(
        URL_SERVER + `/usuario/changepass/${cooperativa.id}`,
        cooperativa,
        config
      );
      dispatch({
